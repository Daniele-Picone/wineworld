...
export async function PUT(req) {
  try {
    ...
    const token = req.headers.get("authorization")?.replace("Bearer ", "");
    if (!token) return NextResponse.json({ error: "Utente non autenticato" }, { status: 401 });

    const { data: { user }, error: userError } = await supabaseServer.auth.getUser(token);
    if (userError || !user) return NextResponse.json({ error: "Utente non valido" }, { status: 401 });
    ...
    return NextResponse.json({ message: "✅ Post aggiornato con successo!" });
  } catch (err) {
    console.error("Errore PUT /api/posts:", err);
